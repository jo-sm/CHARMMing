...
static MOLECULE_T *fgetcif(FILE * fp, char *blockId)
{
    ...
    *l_rname = '\0';
     ...
     */
    ndb_cif_init();
    fprintf(stderr, "back from ndb_cif_init()\n");

    stat = ndb_cif_read_file(fp);
    fprintf(stderr, "back from ndb_cif_read_file()\n");
    ...
    /*  --- need some error processing here!  */
    ...
    return (mol);
#else
    fprintf(stderr, "getcif routine not implemnted in this version!\n");
    fprintf(stderr, "...compile cifparse.a, and NAB with -DMMCIF\n");
    exit(1);
    return (NULL);
#endif
}
