...
class MMPBSA_System(object):
   ...
   def Map(self, receptor_mask=None, ligand_mask=None):
      ...
      if receptor_mask and ligand_mask and not self.stability:
         ...
         for i in range(self.complex_prmtop.ptr('nres')):
            ...
            try:
               self.complex_prmtop.parm_data['RESIDUE_POINTER'][i+1]-1
            except IndexError: # last atom
   ...
   def CheckConsistency(self):
      ...
      # Check the Radii here
      com_radii = 'none'
      rec_radii = 'none'
      lig_radii = 'none'

      try: com_radii = self.complex_prmtop.parm_data['RADIUS_SET'][0]
      except KeyError: pass
      try: rec_radii = self.receptor_prmtop.parm_data['RADIUS_SET'][0]
      except KeyError: pass
      try: lig_radii = self.ligand_prmtop.parm_data['RADIUS_SET'][0]
      except KeyError: pass
      ...
      # Now we scan through each atom. We know which residue the ligand starts
      # on, so we also know which atom number it starts on as well. And we also
      # know how many atoms are in the complex/receptor/ligand.

      ligand_atom_start = self.complex_prmtop.parm_data['RESIDUE_POINTER'][
                                                           self.ligstart - 1]
      for i in range(self.complex_prmtop.ptr('natom')):
         if i < ligand_atom_start - 1:
            lig_or_rec_chg = self.receptor_prmtop.parm_data['CHARGE'][i]
         elif i >= ligand_atom_start - 1 and \
              i < ligand_atom_start - 1 + self.ligand_prmtop.ptr('natom'):
            lig_or_rec_chg = self.ligand_prmtop.parm_data['CHARGE'][
                                            i - ligand_atom_start + 1]
         else:
            lig_or_rec_chg = self.receptor_prmtop.parm_data['CHARGE'][
                                    i - self.ligand_prmtop.ptr('natom')]
         if abs(lig_or_rec_chg-self.complex_prmtop.parm_data['CHARGE'][i])>TINY:
