...
isinerr = 0;
if( gbopt . rungb ){
if( gbopt . igb != 1 && gbopt . igb != 2 && gbopt . igb != 5 ){
fprintf( stderr, "Error: IGB must be 1, 2, or 5!\n" );
isinerr = 1;
}if( gbopt . extdiel <= 0 ){
...
isinerr = 1;
}if( pbopt . solvopt < 1 || pbopt . solvopt > 6 ){
fprintf( stderr, "Error: SOLVOPT must be 1, 2, 3, 4, 5, or 6!\n" );
isinerr = 1;
}if( pbopt . maxitn < 1 ){
...
isinerr = 1;
}if( pbopt . bcopt != 1 && pbopt . bcopt != 5 && pbopt . bcopt != 6 ){
fprintf( stderr, "Error: BCOPT must be 1, 5, or 6!\n" );
isinerr = 1;
...
mm_options( SPRINTF( assert( ( snprintf( NAB_rsbuf, 10000, "istrng=%lf, radiopt=%d, dprob=%lf, iprob=%lf, npbopt=%d", pbopt . istrng * 1000, pbopt . radiopt, pbopt . dprob, pbopt . iprob, pbopt . npbopt ) ) < 10000 ), NAB_strcpy(  &__st0001__, NAB_rsbuf ) ) );
