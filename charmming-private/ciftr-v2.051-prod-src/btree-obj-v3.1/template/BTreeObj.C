...
*/
/* 
  PURPOSE:    Template class of array with AVL binary tree.
              <class TYPE> must have defined operands =, <, ==
*/
#include "BTreeObj.h"
#include <iostream.h>

template<class TYPE> const int BTreeObj<TYPE>::NULL_INDEX  = -1;


template<class TYPE> void BTreeObj<TYPE>::Node::DeleteElement() {
};


template<class TYPE> void BTreeObj<TYPE>::Node::Print() {
...
template<class TYPE> void BTreeObj<TYPE>::Clear() {
  _root = NULL_INDEX;
  _current = NULL_INDEX;
  _data.Clear();
  _deleted.Clear();
...
template<class TYPE> void BTreeObj<TYPE>::Add(TYPE data) {
...
template<class TYPE> void BTreeObj<TYPE>::ReplData(TYPE data)
{
  int target;
  int rmnode;
...
template<class TYPE> int BTreeObj<TYPE>::Seek(TYPE data){
  int target;
  int found = 1;
  int recno;
...
template<class TYPE> int BTreeObj<TYPE>::InTree(TYPE data){
  int target;
  int found = 1;
  int recno;
...
template<class TYPE> void BTreeObj<TYPE>::RecomputeHeightUp(int start_node) {
  int height;
  int finish = 0;
  int tmpnode;
  int hleft, hright;
...
template<class TYPE> void BTreeObj<TYPE>::RecomputeHeightDown(int start_node) {
  int height;
  int tmpnode;
  int hleft, hright;
...
template<class TYPE> void BTreeObj<TYPE>::BalanceTree(int start_node) {
  int pivot;
...
template<class TYPE> void BTreeObj<TYPE>::RotateRight(int node) {
  int left;
...
template<class TYPE> void BTreeObj<TYPE>::RotateLeft(int node) {
  int right;
...
template<class TYPE> void BTreeObj<TYPE>::DoubleRotateRL(int node) {
  RotateRight(_nodes[node].right);
  RotateLeft(node);
}

template<class TYPE> void BTreeObj<TYPE>::DoubleRotateLR(int node) {
...
template<class TYPE> void BTreeObj<TYPE>::GoFirst()
{
  int first;
...
template<class TYPE> void BTreeObj<TYPE>::GoLast()
{
  int last;
...
template<class TYPE> int BTreeObj<TYPE>::MostLeft(int node)
{
  int mostleft;
  mostleft = node;
...
template<class TYPE> int BTreeObj<TYPE>::MostRight(int node)
{
  int mostright;
  mostright = node;
...
template<class TYPE> int  BTreeObj<TYPE>::GoToNext()
{
  int mostright;
  int next = 0;
  int current;
...
template<class TYPE> int  BTreeObj<TYPE>::GoToPrev()
{
  int mostleft;
  int prev = 0;
  int current;
...
template<class TYPE> void BTreeObj<TYPE>::Delete()
{
  int target;
  int rmnode;
  int pivot = NULL_INDEX;
...
template<class TYPE> void BTreeObj<TYPE>:: PrintBTreeInorder(int node) {
/* Inorder */
  if (node!=NULL_INDEX){
    PrintBTreeInorder(_nodes[node].left);
    cout<<"data = ";
    _data[node].Print();cout<<endl;
    _nodes[node].Print();
    PrintBTreeInorder(_nodes[node].right);
  }
}

template<class TYPE> void BTreeObj<TYPE>:: PrintBTreeInorder() {
/* Inorder */
  if (_root!=NULL_INDEX){
    PrintBTreeInorder(_nodes[_root].left);
    cout<<"data = ";
    _data[_root].Print();cout<<endl;
    _nodes[_root].Print();
