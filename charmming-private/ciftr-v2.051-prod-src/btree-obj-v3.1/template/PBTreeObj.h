...
template<class TYPE> class PBTreeObj {
 protected:
  class Node {
  ...
  void Indices(int node,int &i,int*&listind);

  ReVarPCifArray<TYPE> _data;  
  ReVarPCifArray<int> _deleted;  
  ReVarCifArray<Node> _nodes;  
  int _root;
  int _current; 
 ...
 public:
  ...
  ~PBTreeObj() {};
  void Clear();
  ReVarPCifArray<TYPE> *Data() {return &_data;};
  ReVarPCifArray<int> *Deleted() {return &_deleted;};
  inline TYPE GetCurrData() {return _data[_current];};
  inline TYPE* GetCurrDataPtr() {return &(_data[_current]);};
  void Add(TYPE data);
  void ReplData(TYPE data);
  int Seek(TYPE data);
  int InTree(TYPE data);
...
template<class TYPE> PBTreeObj<TYPE>::PBTreeObj() {
  _root = NULL_INDEX;
  _current = NULL_INDEX;
};
