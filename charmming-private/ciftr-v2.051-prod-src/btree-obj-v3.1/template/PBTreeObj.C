...
*/
/* 
  PURPOSE:    Template class of array with AVL binary tree for 
              primitive types.
*/
#include "PBTreeObj.h"
#include <iostream.h>

template<class TYPE> const int PBTreeObj<TYPE>::NULL_INDEX  = -1;


template<class TYPE> void PBTreeObj<TYPE>::Node::DeleteElement() {
};


template<class TYPE> void PBTreeObj<TYPE>::Node::Print() {
...
template<class TYPE> void PBTreeObj<TYPE>::Clear() {
  _root = NULL_INDEX;
  _current = NULL_INDEX;
  _data.Clear();
  _deleted.Clear();
...
template<class TYPE> void PBTreeObj<TYPE>::Add(TYPE data) {
...
template<class TYPE> void PBTreeObj<TYPE>::ReplData(TYPE data)
{
  int target;
  int rmnode;
  int pivot;
...
template<class TYPE> int PBTreeObj<TYPE>::Seek(TYPE data){
  int target;
  int found = 1;
  int recno;
  ...
  if (found==0) {
    ...
    while ((GoToPrev()!=NULL_INDEX) &&(!found)){
      if(_data[target]==_data[_current])
	target=_current;
      else {
	found=1;
	GoToNext();
...
template<class TYPE> int PBTreeObj<TYPE>::InTree(TYPE data){
  int target;
  int found = 1;
  int recno;
...
template<class TYPE> void PBTreeObj<TYPE>::RecomputeHeightUp(int start_node) {
  int height;
  int finish = 0;
  int tmpnode;
  int hleft, hright;
...
template<class TYPE> void PBTreeObj<TYPE>::RecomputeHeightDown(int start_node) {
  int height;
  int tmpnode;
  int hleft, hright;
...
template<class TYPE> void PBTreeObj<TYPE>::BalanceTree(int start_node) {
  int pivot;
...
template<class TYPE> void PBTreeObj<TYPE>::RotateRight(int node) {
  int left;
...
template<class TYPE> void PBTreeObj<TYPE>::RotateLeft(int node) {
  int right;
...
template<class TYPE> void PBTreeObj<TYPE>::DoubleRotateRL(int node) {
  RotateRight(_nodes[node].right);
  RotateLeft(node);
}

template<class TYPE> void PBTreeObj<TYPE>::DoubleRotateLR(int node) {
  RotateLeft(_nodes[node].left);
  RotateRight(node);
}


template<class TYPE> void PBTreeObj<TYPE>::GoFirst()
{
  int first;
...
template<class TYPE> void PBTreeObj<TYPE>::GoLast()
{
  int last;
...
template<class TYPE> int PBTreeObj<TYPE>::MostLeft(int node)
{
  int mostleft;
  mostleft = node;
...
template<class TYPE> int PBTreeObj<TYPE>::MostRight(int node)
{
  int mostright;
  mostright = node;
...
template<class TYPE> int  PBTreeObj<TYPE>::GoToNext()
{
  int mostright;
  int next;
  int current;
...
template<class TYPE> int  PBTreeObj<TYPE>::GoToPrev()
{
  int mostleft;
  int prev;
  int current;
...
template<class TYPE> void PBTreeObj<TYPE>::Delete()
{
  int target;
  int rmnode;
  int pivot;
...
template<class TYPE> void PBTreeObj<TYPE>:: PrintBTreeInorder(int node) {
/* Inorder */
  if (node!=NULL_INDEX){
    PrintBTreeInorder(_nodes[node].left);
    cout<<"data = "<<_data[node]<<endl;
    _nodes[node].Print();
    PrintBTreeInorder(_nodes[node].right);
  }
}

template<class TYPE> void PBTreeObj<TYPE>:: PrintBTreeInorder() {
/* Inorder */
  if (_root!=NULL_INDEX){
    PrintBTreeInorder(_nodes[_root].left);
    cout<<"data = "<<_data[_root]<<endl;
    _nodes[_root].Print();
