...
const int ALLOC_INCR = 5;

class STable {

 protected:
  CifString _tableName;
  int _numColumns;
  unsigned int _numRows;
  ReVarCifArray<CifString> _columnNames;

  int _colAlloc;
  ReVarCifArray<CifString> ** _theData;
 ...
 public:
  STable();
  STable(const char * label);
  virtual ~STable();

  void Rename(const char * label);

  int AddColumn(const char * newColumnName);
  int InsertColumn(const char * newColumnName, int destination);
  int InsertColumn(ReVarCifArray<CifString> & theCol,
  ...
  int RenameColumn(const char * oldColumnName,const char * newColumnName);
  ...
  int FillRowAt(ReVarCifArray<CifString> & theRow, int rowIndex);
  ...
  ReVarCifArray<CifString> * GetSubColumn(int colIndex, int from, int to);
  ReVarCifArray<CifString> * GetSubColumnByIndex(int colIndex, ReVarPCifArray<int> *rowIndex);
  ...
  inline int GetNumColumns() { return _numColumns; };
  inline int GetNumRows() { return _numRows; };
  ReVarCifArray<CifString> * GetColumnNames();
  ReVarCifArray<CifString> * GetItemNames(); // a particular form of column name
  inline const char * GetName() { return _tableName.Text(); }; 
  
  void Copy(STable *st);
};

inline STable::STable() { Clear(); }
inline STable::~STable() { Delete(); }

inline STable::STable(const char * label) {
  Clear();
  if (label)
    _tableName.Copy(label);
  else _tableName.Clear();
}
