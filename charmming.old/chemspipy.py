...
TOKEN  = '15252a8d-a7dd-4c56-8678-26833c49f3e8'
...
def asyncfind(query):
    """ Search by Name, SMILES, InChI, InChIKey, etc. Returns first 100 Compounds """
    assert type(query) == str or type(query) == unicode, 'query not a string object'
    searchurl = 'http://www.chemspider.com/Search.asmx/AsyncSimpleSearch?query=%s&token=%s' % (urllib2.quote(query), TOKEN)
    response = urllib2.urlopen(searchurl)
    tree = ET.parse(response)
    elem = tree.getroot()
    rid_tags = elem.getiterator('{http://www.chemspider.com/}string')
    for tag in rid_tags:
        rid=tag.text
...
def getAsyncSearchResult(rid):
    """ Search by Name, SMILES, InChI, InChIKey, etc. Returns first 100 Compounds """
    assert type(rid) == str or type(rid) == unicode, 'query not a string object'
    searchurl = 'http://www.chemspider.com/Search.asmx/GetAsyncSearchResult?rid=%s&token=%s' % (urllib2.quote(rid), TOKEN)
    response = urllib2.urlopen(searchurl)
    tree = ET.parse(response)
    elem = tree.getroot()
    csid_tags = elem.getiterator('{http://www.chemspider.com/}int')
