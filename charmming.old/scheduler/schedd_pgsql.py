...
import sys, os, time, string, select, getopt, signal
...
class tracker:
   ...
   def updateStatus(self):
      ...
      try:
         dba = psycopg2.connect(host="localhost", user="charmming", password="charmming", database="charmming")
         dba.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
      except:
         self.logif.write_log(0, "A: Authentication to PostgresQL db failed!")
         return None
      ...
      for row in rset:
         ...
         if currstate != j_state:
            ...
            if currstate >= 2:
               if currstate == 2:
                  sstat2 = "UPDATE job_scheduler SET started=%s WHERE id=%d" % (psycopg2.Timestamp(ltime[0],ltime[1],ltime[2],ltime[3],ltime[4],ltime[5]),j_id)
               elif currstate == 3 or currstate == 4:            
                  sstat2 = "UPDATE job_scheduler SET ended=%s WHERE id=%d" % (psycopg2.Timestamp(ltime[0],ltime[1],ltime[2],ltime[3],ltime[4],ltime[5]),j_id)
               try:
                  crb.execute(sstat2)
                  crb.close()
               except:
   ...
   def lookupJobStatus(self,jid):
      ...
      try:
         dbc = psycopg2.connect(host="localhost", user="charmming", password="charmming", database="charmming")
         dbc.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
         crc = dbc.cursor()
      except:
         self.logif.write_log(0, "B: Authentication to PostgresQL db failed!")
         return None
   ...
   def killJob(self,request):
      ...
      try:
         ...
         self.logif.write_log(0, "C: Authentication to PostgresQL db failed!")
         return -1
   ...
   def startJob(self,request,execs,nprocs):
      ...
      try:
         ...
         self.logif.write_log(0, "D: Authentication to PostgresQL db failed!")
         return None
      ...
      for i in range(len(scripts)):
         ...
         sqlstat += "(%d,'%s','%s',0,'%s','%s',%s)" % (user,schedids[i],batchid,exelist[i],scripts[i],psycopg2.Timestamp(ltime[0],ltime[1],ltime[2],ltime[3],ltime[4],ltime[5]))
         crc.execute(sqlstat)
