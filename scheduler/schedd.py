...
import charmming_config #this is to get the apps...
...
class tracker:
    ...
    def updateStatus(self):
        ...
        sqlstat = "SELECT id,sched_id,state,script,batchid,exe FROM job_scheduler WHERE ended is null"
        try:
            cra.execute(sqlstat)
        except:
        ...
        for row in rset:
            ...
            if currstate != j_state:
                ...
                if currstate == 3:
                    ...
                    check_apps = {'charmm':charmming_config.apps['charmm'],'charmm-mscale':charmming_config.apps['charmm-mscale'],'charmm-apbs':charmming_config.apps['charmm-apbs'],\
                                'qchem':charmming_config.apps['qchem']}
                    if j_exe in check_apps.values(): #do this first so we know whether to check
                        if j_exe == charmming_config.apps['charmm'] or j_exe == charmming_config.apps['charmm-mscale'] or j_exe == charmming_config.apps['charmm-apbs']:
                            self.logif.write_log(4, "Checking for normal termination in %s." % j_outfile)
                            basecmd = "grep -iw 'normal termination'"
                        elif j_exe == charmming_config.apps['qchem']:
                            self.logif.write_log(4, "Checking for Have a nice day in %s." % j_outfile)
                            basecmd = "grep -iw 'Thank you very much for using Q-Chem.  Have a nice day.'"
                        else:
                            pass #we dunno what else to do yet here
                        cmd = "%s %s" %(basecmd,j_outfile)
