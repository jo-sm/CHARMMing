...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding unique constraint on 'ProgramSet', fields ['slug']
        db.create_unique('charmming_programset', ['slug'])

        # Adding unique constraint on 'ProgramSet', fields ['name']
        db.create_unique('charmming_programset', ['name'])

        # Adding unique constraint on 'Group', fields ['name']
        db.create_unique('charmming_group', ['name'])

        # Adding unique constraint on 'Task', fields ['slug', 'program_set']
        db.create_unique('charmming_task', ['program_set_id', 'slug'])

        # Adding unique constraint on 'ProgramParameter', fields ['name', 'program']
        db.create_unique('charmming_programparameter', ['name', 'program_id'])

        # Adding unique constraint on 'User', fields ['username']
        db.create_unique('charmming_user', ['username'])

        # Adding unique constraint on 'User', fields ['email_address']
        db.create_unique('charmming_user', ['email_address'])

        # Adding unique constraint on 'Program', fields ['name']
        db.create_unique('charmming_program', ['name'])


    def backwards(self, orm):
        # Removing unique constraint on 'Program', fields ['name']
        db.delete_unique('charmming_program', ['name'])

        # Removing unique constraint on 'User', fields ['email_address']
        db.delete_unique('charmming_user', ['email_address'])

        # Removing unique constraint on 'User', fields ['username']
        db.delete_unique('charmming_user', ['username'])

        # Removing unique constraint on 'ProgramParameter', fields ['name', 'program']
        db.delete_unique('charmming_programparameter', ['name', 'program_id'])

        # Removing unique constraint on 'Task', fields ['slug', 'program_set']
        db.delete_unique('charmming_task', ['slug', 'program_set_id'])

        # Removing unique constraint on 'Group', fields ['name']
        db.delete_unique('charmming_group', ['name'])

        # Removing unique constraint on 'ProgramSet', fields ['name']
        db.delete_unique('charmming_programset', ['name'])

        # Removing unique constraint on 'ProgramSet', fields ['slug']
        db.delete_unique('charmming_programset', ['slug'])
    ...
    models = {
        ...
        'charmming.programparameter': {
            'Meta': {'unique_together': "(('name', 'program'),)", 'object_name': 'ProgramParameter'},
        ...
        'charmming.task': {
            'Meta': {'unique_together': "(('slug', 'program_set'),)", 'object_name': 'Task'},
            'description': ('django.db.models.fields.CharField', [], {'max_length': '150'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'charmming.taskfieldoption': {
            ...
            'field': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['charmming.TaskField']"}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'charmming.user': {
            ...
            'email_address': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '65'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
