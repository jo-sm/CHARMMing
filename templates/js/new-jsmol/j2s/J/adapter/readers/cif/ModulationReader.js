...
Clazz.load (["J.adapter.smarter.AtomSetCollectionReader"], "J.adapter.readers.cif.ModulationReader", ["java.lang.Float", "java.util.Hashtable", "J.util.BSUtil", "$.Escape", "$.JmolList", "$.Logger", "$.Matrix3f", "$.Modulation", "$.ModulationSet", "$.P3", "$.SB", "$.V3"], function () {
...
if (!this.modVib && !this.modAverage) this.addJmolScript ("modulation on");
...
if (this.modType != null) switch (id.charAt (0)) {
case 'O':
case 'D':
case 'U':
if (this.modType.indexOf (id.charAt (0)) < 0 || this.modSelected > 0 && this.modSelected != 1) return;
break;
}
...
if (iop < 0) iop = 0;
if (J.util.Logger.debuggingHigh) J.util.Logger.debug ("\nsetModulation: i=" + a.index + " " + a.atomName + " xyz=" + a + " occ=" + a.occupancy);
if (iop != this.iopLast) {
this.iopLast = iop;
...
}var vocc0 = a.occupancy / 100;
var ms =  new J.util.ModulationSet (a.index + " " + a.atomName, J.util.P3.newP (a), vocc0, this.modDim, list, this.gammaE, this.gammaIS, this.q123, this.qlen);
a.vib = ms;
ms.calculate ();
if (!Float.isNaN (ms.vocc)) {
if (this.modVib && !Float.isNaN (vocc0)) {
a.occupancy = Clazz.floatToInt ((vocc0 + ms.vocc) * 100);
} else if (ms.vocc < 0.5) {
a.occupancy = 0;
