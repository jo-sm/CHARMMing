...
GLmol.prototype.parsePDB2 = function(str) {
   ...
   for (var i = 0; i < lines.length; i++) {
      ...
      if (recordName == 'ATOM  ' || recordName == 'HETATM') {
         var atom, resn, chain, resi, x, y, z, hetflag, elem, serial, solvflag, ionflag, altLoc, b;
         altLoc = line.substr(16, 1);
	 solvflag = false; // All atoms are assumed to not be solvents unless proven otherwise
	 ionflag = false; // Same with ion flag.
         if (altLoc != ' ' && altLoc != 'A') continue; // FIXME: ad hoc
         serial = parseInt(line.substr(6, 5));
         atom = line.substr(12, 4).replace(/ /g, "");
         resn = line.substr(17, 3); 
	 chain = line.substr(21, 1);
	 if (chain == ' ') { //Accounting for "normal" format PDB files
		chain = line.substr(72, 1);
	 }
...
//Ensures ions render separately from solvent
GLmol.prototype.getIons = function(atomlist) {
   var ret = [];
   for (var i in atomlist) {
	var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;

	if (atom.ionflag) ret.push(atom.serial);
   }
   return ret;
};
...
GLmol.prototype.removeSolvents = function(atomlist) {
   ...
   for (var i in atomlist) {
      ...
      if ( !(atom.solvflag)){ ret.push(atom.serial); }
   }
   return ret;
};
...
GLmol.prototype.getProteins = function(atomlist) {
   ...
   for (var i in atomlist) {
      ...
      if ((!atom.hetflag) && (!atom.ionflag)) ret.push(atom.serial);
   }
   return ret;
};
...
// for HETATM only; returns solvent atoms, NOT ions
GLmol.prototype.getNonbonded = function(atomlist, chain) {
   var ret = [];
   for (var i in atomlist) {
      var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;

      if (atom.hetflag && atom.solvflag) ret.push(atom.serial);
   }
   return ret;
};
